...
bootstrap <- function(data, formula, conf = 0.95, R = 100, ntree = 500, data_split = c(0.7, 0.3)){
  ...
  set.seed(12345)
  # Divide data into train, calibration and test sets 
  n = dim(data)[1]
  ...
  rng = function(data, mle){
    ...
    data_gen[,y] = rnorm(n, mle$prediction, sd(mle$residual)) # should sd be the true value?
    return(data_gen)
  }
  ...
  prediction = function(data_gen, pars){
    ...
    predict_expected = predict(model, newdata = test)
    n = dim(test)[1]
    
    # simulate prediction with added Gaussian noise
    sim = rnorm(n, predict_expected, sd(pars$residual)) # should sd be the true value?
    return(sim)
  }
